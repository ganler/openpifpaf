...
struct Occupancy {
  ...
  : d0(d0_), d1(d1_), d2(d2_), occupancy_view(d0_*d1_*d2_) {}
...
static const float seedThreshold = 0.5f;
  ...
  for (size_t i = 0; i < x.size(); ++i) {
    ...
    for (int64_t xx = minx; xx < maxx; ++xx) {
      ...
      for (int64_t yy = miny; yy < maxy; ++yy) {
        ...
        float vv = (deltax2 < 0.25 && deltay2 < 0.25) ? \
          cv : cv * approx_exp(-0.5 * (deltax2 + deltay2) / (csigma * csigma));
...
std::array<std::vector<float>, 9>
  OpenPifPafPostprocessor::pafCenter(const std::array<std::vector<float>, 9>& paf_field,
                                     float x, float y, float sigma)
{
  std::array<std::vector<float>, 9> filtered;
  const int paf_stride = paf_field.front().size();
  for (int i = 0; i < paf_stride; ++i) {
    if( (paf_field[1][i] < x - sigma) ||
        (paf_field[1][i] > x + sigma) ||
        (paf_field[2][i] < y - sigma) ||
        (paf_field[2][i] > y + sigma))
                      continue;
    ...
    // float score = std::exp(-0.5 * d2 / sigma2) * paf_field[0][i];
      for (size_t j = 0; j < filtered.size(); ++j) {
        filtered[j].push_back(paf_field[j][i]);
      }
  }
  return filtered;
}
  ...
  OpenPifPafPostprocessor::growConnectionBlend(float x, float y, float s, const std::array<std::vector<float>, 9>& paf_field)
{
  ...
  // const auto paf_field = pafCenter(paf_field_, x, y, sigma_filter);
  const int paf_stride = paf_field.front().size();
  for (int i = 0; i < paf_stride; ++i) {
    if( (paf_field[1][i] < x - sigma) ||
        (paf_field[1][i] > x + sigma) ||
...
struct queue_item { // -score, xyv, start_i, end_i
    ...
    queue_item(Args&& ... args) : data(std::make_tuple(std::forward<Args>(args)...)) {}
                                   ...
                                   const FBContainer& pafBackward)
{
  ...
  for (size_t joint_i = 0; joint_i < numKeypoints; ++joint_i) {
  ...
  while (true) {
    ...
    if (ann.keypoints[jti * 3 + 2] > 0.0) continue;
  ...
  const std::vector<float>& paf)
{
  ...
  for (size_t field_i = 0; field_i < 17; ++field_i) {
    ...
    for (size_t hw_index = 0; hw_index < hw_size; ++hw_index) {
      ...
      if (pif[vindex] > seedThreshold) {
        ...
        if (v > seedThreshold) {
          ...
          seeds.push_back(std::make_tuple(v, field_i, x * STRIDE, y * STRIDE, s * STRIDE));
        }
      }
    }
  }
  ...
  for (const auto& [v, f, x, y, s] : seeds) {
      // std::cout << "--> REPORT: " << v << ' ' << f << ' ' << x << ' ' << y << ' ' << s << ' ' << occupied.get(f, y, x) << '\n'; 
      ...
      for (int i = 0; i < numKeypoints; ++i) {
        ...
        scalarSquareAddSingle(occupied, i, H_hr, W_hr, ax, ay, width, occupied.reduction, occupied.min_scale_reduced); // width is sigma...
      }
  }
